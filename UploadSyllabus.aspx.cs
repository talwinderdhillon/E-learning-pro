...
namespace e_learning
{
    public partial class WebForm15 : System.Web.UI.Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            string path = Server.MapPath("/Syllabus/" + filename);
            if (FileUpload1.HasFile)
            {
                FileUpload1.SaveAs(path);
                connection.cmd = new SqlCommand("insert into Syllabus values(" + connection.userid + "," + Int32.Parse(DropDownList1.SelectedValue.ToString()) + ",'" + filename + "')");
                connection.cmd.Connection = connection.con;
